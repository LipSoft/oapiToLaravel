...
Object.entries(schema['x-ls'].belongsTo).map(([constraint, constraintValue]) => (
  { ...constraintValue,
    lowerCaseReferencedTableName: constraintValue.referencedTableName ? constraintValue.referencedTableName.toLowerCase() : null }
)).filter(x => x.referencedTableName != null)
...
Object.entries(schema.properties).map(([property, propertyValue]) => ({ field: property, desc: propertyValue['x-ls'].desc })).filter(field => fieldIsFK(field, openapi))
