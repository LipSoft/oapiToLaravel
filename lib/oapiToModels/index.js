...
Object.entries(schema['x-ls'].belongsTo).map(([constraint, a]) => a).filter(x => x.referencedTableName != null)

const hasManyFromSchema = schema => schema && schema.type === 'object' && schema['x-ls'] && schema['x-ls'].hasMany &&
Object.entries(schema['x-ls'].hasMany).map(([constraint, a]) => a).filter(x => x.referencedTableName != null)
